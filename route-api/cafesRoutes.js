...
cafesRouter.post('/attractions/cafes/:id',async (req,res) => {
   ...
   const rating = estimations.reduce((accum,number) => accum + number,Number(req.body.estimate))/(estimations.length + 1);
   const cafe = await Cafes.findByIdAndUpdate(req.params.id , {$push: { 'estimations': Number(req.body.estimate) }, 'rating': rating}, {new: true});
   res.send(cafe); 
})
